...
public interface ProductSizeRepository extends CrudRepository<ProductSize,Long> {
    ...
    @Query(value = "select s.id , s.value, ps.count, ps.id as psid, re.name, pr.id as prid from sizes as s left join product_size as ps on ps.size_id = s.id and ps.product_id = :productId join product as pr on ps.product_id = pr.id join reservoir re on pr.reservoir_id = re.id", nativeQuery = true)
